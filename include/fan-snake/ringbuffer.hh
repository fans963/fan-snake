...
public:
    ...
    T* front() {
        ...
        if (out == in_.load(std::memory_order::acquire)) return nullptr;
        else
    ...
    T* back() {
        ...
        if (in == out_.load(std::memory_order::relaxed)) return nullptr;
        else
    ...
    size_t emplace_back_multi(F construct_functor, size_t count = -1) {
        ...
        if (count > writeable) count = writeable;
        if (!count) return 0;
    ...
    size_t push_back_multi(F generator, size_t count = -1) {
        return emplace_back_multi(
            [&](std::byte* storage) { new (storage) T { generator() }; }, count);
    }
